...
avgShannDist <- taskTimeShan %>% dplyr::summarize(avg = mean(shanDist, na.rm = T)) %>% dplyr::pull(avg)
...
```{r}
# much closer to normal than bee shannons, maybe okay to do a single model rather than two-part?
ggdensity(taskS$shannon)
qqp(taskS$shannon, "norm")
taskS

# need to add natal colony as random effect
ps0 <- glmer(shannon ~1 + (1|nestID),
              data = taskS)
ps1 <- glmer(shannon ~ behav + nestTYPE + nBehav + (1|nestID),
              data = taskS)
ps2 <- glmer(shannon ~ behav + nestTYPE + (1|nestID),
              data = taskS)
ps3 <- glmer(shannon ~ behav + nBehav + (1|nestID),
              data = taskS)
ps4 <- glmer(shannon ~ nestTYPE + nBehav + (1|nestID),
              data = taskS)
ps5 <- glmer(shannon ~ behav + (1|nestID),
              data = taskS)
ps6 <- glmer(shannon ~ nestTYPE + (1|nestID),
              data = taskS)
ps7 <- glmer(shannon ~ nBehav + (1|nestID),
              data = taskS)

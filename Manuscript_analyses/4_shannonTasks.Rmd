...
shanEarly_data <- read.csv("../Data/nestEarlyProp_data.csv")
shanLate_data <- read.csv("../Data/nestLateProp_data.csv")
...
taskS <- matrix(NA, ncol = 3)
colnames(taskS) <- c("nestID", "behav", "shannon")
...
for(index in 1:length(unique(taskP$nestID))) {                      # for each unique nestID in the dataset
  ...
  nest_m <- nest_m[rowSums(nest_m) > 0,]                            # remove behaviors without any observations
  # make a matrix to put shannon values into
  nestS <- matrix(NA, ncol = 3, nrow = nrow(nest_m))                # make a nest-specific matrix to put shannon values into
  nestS <- as.data.frame(nestS)                                     # turn it into a df
  colnames(nestS) <- c("nestID", "behav", "shannon")    # name columns same as the taskP df
  shannon <- diversity(nest_m, index = "shannon")                   # calculate shannon for each behavior in the nest
  nestS$behav <- names(shannon)                                     # add the name of each behavior to the nestS df
...
# plot it
ggplot(taskS, aes(shannon, behav, color = nestTYPE)) +
...
ggplot(dol, aes(shannon, nBehavs, color = nestTYPE, shape = caste)) +
  geom_point(position = position_jitter(width=0.05, height=1), alpha = 0.8) +
  scale_shape_manual(values = c(21, 16))
```

# POOLED TIMEPOINTS, including egg laying, combined foraging -- looks very similar to split foraging
<!-- yields df: dol -->
...
<!-- # data is transformed such that all values for a given nest sum to 1 -->
<!-- dol <- shanP_data -->
<!-- dol$allforaging <- dol$nectarforaging + dol$pollenforaging -->

<!-- # convert to matrix of behaviors to be used in shannon -->
<!-- dol_m <- as.matrix(dol[,c("feedsbrood", "allforaging", "egglaying")]) -->
...
# COMPARE EARLY VS LATE TIMEPOINTS, excluding egg laying, combined all foraging -- looks very similar to split foraging
<!-- ```{r} -->
<!-- dolT <- shanT_data -->
<!-- dolT$allforaging <- dolT$nectarforaging + dolT$pollenforaging -->

<!-- # convert to matrix of behaviors to be used in shannon -->
<!-- dolT_m <- as.matrix(dolT[,c("feedsbrood", "allforaging")]) -->
...
ps <- glmer(shannon ~ nestTYPE + caste + (1|nestID),
              data = dol)
mymodel <- ps
summary(mymodel)
tab_model(mymodel)
...
```{r}
# it's not really normal... not sure what distribution to use
ggdensity(dolT$shannon)
qqp(dolT$shannon, "norm")

# need to add natal colony as random effect
ts <- glmer(shannon ~ timepoint + nestTYPE + caste + (1|nestID),
              data = dolT)
mymodel <- ts
summary(mymodel)
tab_model(mymodel)
...
# need to add natal colony as random effect
cs <- glmer(shanDist ~ nestTYPE + caste + (1|nestID),
              data = dolT)
mymodel <- cs
summary(mymodel)
tab_model(mymodel)

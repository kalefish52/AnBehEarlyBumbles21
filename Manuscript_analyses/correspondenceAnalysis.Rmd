...
### check out what queens are doing
```{r}
count_queen <- count_data[count_data$caste == "queen",] 
count_queen <- count_queen[order(count_queen$nBehavs, decreasing = TRUE),]
count_queen
# NMDS on queens only
...
nmds_data <- count_queen[1:(nrow(count_queen)-3),]
# make NA values 0
...
nmds_all <- as.matrix(nmds_data[,7:(ncol(nmds_data) - 1)])
...
ggplot() + 
  ...
  geom_point(data=data.scores, aes(x=NMDS1, y=NMDS2, colour=nestTYPE, shape=timepoint), 
             size = 3, position = position_jitter(width=0.03, height=0.03)) + 
  # scale_shape_manual(values = c(21, 16)) +
  geom_line(data=data.scores, aes(x=NMDS1, NMDS2, fill = beeID), size = 0.35, position = position_jitter(width = 0, height = 0), 
           arrow = arrow(length=unit(0.25,"cm"), type = "open")) +
  coord_cartesian(xlim=c(-2.25, 1), ylim=c(-1.5, 1.1))
```

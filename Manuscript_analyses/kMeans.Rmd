...
```{r}
## assumes the working directory is the project folder ./SarroFisher
count_data <- read.csv("./Data/tempData/count_stats.csv", sep = ",")
count_data
```
...
count_data$totalForaging <- count_data$pollenforaging + count_data$nectarforaging
data <- count_data %>% select(egglaying, feedsbrood, nectarforaging, pollenforaging)
...
plot_ly(x = count_data$feedsbrood, 
        y = count_data$egglaying, 
        z = count_data$totalForaging, 
        type="scatter3d", 
        mode="markers",
  ...
  layout(
    ...
    scene = list(
      ...
      yaxis = list(title = "egglaying"),
      zaxis = list(title = "totalForaging"))
  )
```

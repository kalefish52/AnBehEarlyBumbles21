...
```{r}
# remove bees with NA beeID
workerNMDS <- nmds_data[!is.na(nmds_data$beeID),]
# remove queens 
workerNMDS <- workerNMDS[workerNMDS$caste == "worker",]
workerNMDS
  
# make matrix of only behavior counts for all timepoints
nmds_worker <- as.matrix(workerNMDS[, c("feedsbrood", "nectarforaging", "pollenforaging", "egglaying")]) 
nmds_worker

# Run NMDS with euclidean distance
nmds_worker <- metaMDS(nmds_worker)
plot(nmds_worker)

# Get coordinates to graph it 
# Gets NMDS info for each individual
data.scores <- as.data.frame(scores(nmds_worker))
data.scores$caste <- workerNMDS$caste
data.scores$nestTYPE <-workerNMDS$nestTYPE
data.scores$beeID <-workerNMDS$beeID
data.scores$nestID <- workerNMDS$nestID
data.scores$nBees <- workerNMDS$nBees
data.scores$nWork <- workerNMDS$nWork
data.scores$queen <- workerNMDS$queen
data.scores

# Gets NMDS info for each behavior
behav.scores<-as.data.frame(scores(nmds_worker, "species"))
behav.scores$behav<-rownames(behav.scores)
behav.scores

data.scores$nestTYPE <- ordered(data.scores$nestTYPE, levels = c("NT", "QT", "NF", "QF"))

# add ovary stage and body size into worker NMDS plot
data.scores$beeID <- paste0(data.scores$nestID, data.scores$beeID)
data.scores$avgStage <- NA
data.scores$size <- NA
for (i in 1:nrow(data.scores)) {
  for (j in 1:nrow(Scores)) {
    if (data.scores$beeID[i] == Scores$beeID[j]) {
      data.scores$avgStage[i] <- Scores$avgStage[j]
      data.scores$size[i] <- Scores$size[j]
    }
  }
}
# add ovary stage and body size into workerNMDS df
workerNMDS$beeID <- paste0(workerNMDS$nestID, workerNMDS$beeID)
workerNMDS$avgStage <- NA
workerNMDS$size <- NA
for (i in 1:nrow(workerNMDS)) {
  for (j in 1:nrow(Scores)) {
    if (workerNMDS$beeID[i] == Scores$beeID[j]) {
      workerNMDS$avgStage[i] <- Scores$avgStage[j]
      workerNMDS$size[i] <- Scores$size[j]
    }
  }
}
...
ggplot() + 
  geom_point(data=data.scores, aes(x=NMDS1, y=NMDS2, color = avgStage), size = 2, position = position_jitter(width=0.05, height=0.05), alpha = 0.6) + 
...
ggplot() +
  geom_point(data = workerNMDS, aes(x = egglaying, y = avgStage, color = avgStage), 
...
ggplot() + 
  geom_point(data=data.scores, aes(x=NMDS1, y=NMDS2, color = size), size = 2, position = position_jitter(width=0.05, height=0.05), alpha = 0.6) + 
...
ggplot() +
  geom_point(data = workerNMDS, aes(x = egglaying, y = size, color = size), 

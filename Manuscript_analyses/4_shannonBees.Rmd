...
# bees with >3 observations, regardless of # observations per nest
...
# nestTYPE vs. shannon scatterplot
...
for (i in 1:nrow(specPool)) {
  specPool$specBehav[i] <- names(specPool[skip + max.col(specPool[i, c("feedsbrood", "nectarforaging", "pollenforaging", "egglaying")])])
}
specPool
...
nSpec <- matrix(NA, ncol = 3, nrow = length(unique(dol$nestID))*4)
# convert to df
...
nSpec$specBehav <- c("feedsbrood", "nectarforaging", "pollenforaging", "egglaying")
...
for (i in 0:(length(unique(dol$nestID))-1)) {
  nSpec$nestID[((i * 4) + 1):((i + 1) * 4)] <- unique(dol$nestID)[i + 1]
}
# replace number of bees > 0 with accurate number
# first summarize 
nSpec_missing0 <- specPool %>% dplyr::group_by(nestID, nestTYPE, specBehav) %>% dplyr::summarise(nBeesPerNest = dplyr::n())
nSpec_missing0
...
# count number of specialists in each category
# make empty matrix to put number into
nSpecT <- matrix(NA, ncol = 4, nrow = length(unique(dolT$nestID))*6)
# convert to df
nSpecT <- as.data.frame(nSpecT)
# name columns relevant info
colnames(nSpecT) <- c("nestID", "timepoint", "specBehav", "nBeesPerNest")
# repeat the 4 behaviors over in specBehav column
nSpecT$specBehav <- c("feedsbrood", "feedsbrood", "nectarforaging", "nectarforaging", "pollenforaging", "pollenforaging")
...
for (i in 0:(length(unique(dolT$nestID))-1)) {
  nSpecT$nestID[((i * 6) + 1):((i + 1) * 6)] <- unique(dolT$nestID)[i + 1]
}
# replace number of bees > 0 with accurate number
# first summarize 
nSpec_missing0T <- specTime %>% dplyr::group_by(nestID, nestTYPE, specBehav, timepoint) %>% dplyr::summarise(nBeesPerNest = dplyr::n())
nSpec_missing0T
...
ggplot(nSpecT, aes(nBeesPerNest, specBehav, shape = timepoint)) +
  geom_point(position = position_jitter(width=0.18, height=0.2), alpha = 0.7, aes(color = nestTYPE, fill = nestTYPE)) +

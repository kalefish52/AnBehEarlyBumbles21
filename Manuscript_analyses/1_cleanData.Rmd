...
#knitr::opts_chunk$set(fig.width=2.5, fig.height=3.5) # use only to create multipanel shannon task plot
knitr::opts_chunk$set(fig.width=2.5, fig.height=4.5) # use only to create multipanel shannon bee plot
# knitr::opts_chunk$set(fig.width=3, fig.height=3.5) # use only to create multipanel change in shannon plot
```
# Load Packages
```{r}
...
#.
# Scale data to nestBehavs and Filter Data
## label lazy workers
```{r}
# define lazy threshold
obsThres <- 3
...
# make shannon NA for lazy bees with less than threshold number of behaviors - we only want to include bees with above a threshold number in shannon analyses
count_data$shannon[count_data$lazy == "lazy"] <- NA
...
p1 <- ggplot(count_data, aes(shannon, fill = nestTYPE)) + 
  geom_histogram(color = "black", alpha = 0.7) +
  labs(x= "Bee Shannon diversity index", y = "Count", fill = "Social configuration") +
  scale_fill_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99"), labels = c("W3", "W5", "QW3", "QW5"))
p1

# plot nestTYPE vs. shannon scatterplot
p2 <- ggplot(count_data, aes(shannon, nestTYPE, color = nestTYPE, shape = caste)) +
  geom_point(position = position_jitter(width=0.02, height=0.2), alpha = 0.7) +
  ...
  scale_y_discrete(labels=c("NT" = "W3", "NF" = "W5", "QT" = "QW3", "QF" = "QW5")) +
  labs(x = "Bee Shannon diversity index", y = "Social configuration", color = "Social configuration", shape = "Caste")
p2
ggsave("../figures/shannon.jpg")

# plot number of behaviors vs. shannon scatterplot
p3 <- ggplot(count_data, aes(shannon, nBehavs, color = nestTYPE, shape = caste)) +
  geom_point(position = position_jitter(width=0.02, height=3), alpha = 0.7) +
  scale_shape_manual(values = c(21, 16), labels = c("Queen", "Worker")) +
  scale_color_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99"), labels = c("W3", "W5", "QW3", "QW5")) +
  labs(x = "Bee Shannon diversity index", y = "Number of observed behaviors", color = "Social configuration", shape = "Caste")
p3
ggsave("../figures/shannonVSnBehavs.jpg")

ggarrange(p1, p2, p3, nrow = 3, labels = c("A", "B", "C"))
ggsave("../figures/shannonBee.jpg")
...
print(paste(nrow(count_known[count_known$nBehavs == 0,]), "out of", nrow(count_known), "known bees 0 recorded behaviors", sep = " "))

print(paste(nrow(count_known[count_known$nBehavs == 1,]), "out of", nrow(count_known), "known bees 1 recorded behaviors", sep = " "))

print(paste(nrow(count_known[count_known$nBehavs == 2,]), "out of", nrow(count_known), "known bees 2 recorded behaviors", sep = " "))
...
count_time$shannon[count_time$lazy == "lazy"] <- NA
# make shannon NA for unidentified bees - we don't want to inclue these in analyses
count_time[grepl("unknown", count_time$beeID), "shannon"] <- NA

count_time$nestTYPE <- ordered(count_time$nestTYPE, levels = c("NT", "NF", "QT", "QF"))
# plot shannon values by timepoint
p1 <- ggplot(count_time, aes(shannon, nestTYPE, shape = interaction(caste, timepoint), fill = nestTYPE, color = nestTYPE)) + 
  geom_point(position = position_jitter(width=0.15, height=0.2), alpha = 0.8) +
  scale_shape_manual(values = c(2, 17, 0, 15)) +
  stat_summary(fun = mean, geom = "point", size = 5) +
  labs(shape = "caste*timepoint")
p1
...
avgShannDist <- mean(count_time$shanDist, na.rm = T)

p2 <- ggplot(count_time, aes(shanDist, nestTYPE, color = nestTYPE, shape = caste)) + geom_vline(aes(xintercept = avgShannDist), color = "gray70", size = 0.6) +
...
ggsave("../figures/shannonBeeTime.jpg", width=4.5, height=6.5)
...
for (i in 1:nrow(count_data)) {
  # if the bee has fewer than threshold number of obs (shannon is NA as well)
  if (count_data$lazy[i] == "lazy") {
    # name the behavior as lazy
    count_data$specBehav[i] <- "lazy"
    # kepp proportion as NA
    count_data$specProp[i] <- NA
  }
  else if (is.na(count_data$shannon[i])) {
  # if the shannon is na, skip it
  }
  ...
  # if the bee is not a specialist (i.e. shannon > threshold)
    ...
    # name the behavior that each individual did the most of: column index of max number of behaviors + skip columns
    behavior <- names(count_data[skip + max.col(count_data[i, c("nectarforaging", "feedsbrood", "pollenforaging", "egglaying")])])
    # calculate proportion of total behaviors comprised by the most common behavior
    count_data$specProp[i] <- count_data[i, behavior] / count_data$nBehavs[i]
  }
}
...
minSpecProp <- min(count_data$specProp[count_data$shannon <= shannonSpecialist], na.rm = TRUE)
...
# check the range of proportions of most common behavior carried out by generalists
minGenProp <- min(count_data$specProp[count_data$shannon > shannonSpecialist], na.rm = TRUE)
maxGenProp <- max(count_data$specProp[count_data$shannon > shannonSpecialist], na.rm = TRUE)
print(paste("Generalists carry out", minGenProp, "to", maxGenProp, "percent of their most common behavior.", sep = " "))

# count the number of bees that categorize as specialists, generalists, and lazy
count_known <- count_data[!grepl("unknown", count_data$beeID), ]
count_notNA <- count_known[!is.na(count_known$shannon),]
print(paste("there are", nrow(count_notNA[count_notNA$shannon <= shannonSpecialist, ]), "specialists,", nrow(count_notNA[count_notNA$shannon > shannonSpecialist, ]), "generalists, and", nrow(count_known[count_known$specBehav == "lazy", ]), "lazy workers.", sep = " "))
# tally the number of specialists in each category
specCat <- count_known %>% dplyr::group_by(specBehav) %>% dplyr::summarise(nSpec = dplyr::n())
specCat

# remove unknown workers
...
nSpec %>% filter(specBehav=="nectarforaging") %>% nrow()
...
for (i in 1:nrow(count_time)) {
   # if the bee has fewer than threshold number of obs (shannon is also NA)
  if (count_time$lazy == "lazy") {
    # name the behavior as lazy
    count_time$specBehav[i] <- "lazy"
    # kepp proportion as NA
    count_time$specProp[i] <- NA
  }
  else if (is.na(count_time$shannon[i])) {
  # if shannon is na, skip it, stays as NA specBehav
  }
  # if the bee is a specialist (i.e. shannon <= threshold)
...
# remove unknown workers
count_time <- count_time[!grepl("unknown", count_time$beeID),]
...
# remove unknown workers (with NA specialist behavior)
nSpecTime_missing0 <- nSpecTime_missing0[!is.na(nSpecTime_missing0$specBehav),]
nSpecTime_missing0
nSpecTime
...
p1 <- ggplot(taskShan, aes(shannon, behav)) +
  geom_point(position = position_jitter(width=0.05, height=0.2), alpha = 0.7, aes(color = nestTYPE)) +
  scale_color_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99"), labels = c("W3", "W5", "QW3", "QW5")) +
...
prop_bees
prop_gen <- prop_bees[prop_bees$specBehav == "generalist", c("beeID", "nectarforaging", "feedsbrood", "pollenforaging", "egglaying", "nBehavs", "shannon", "specBehav")]
max(prop_gen$nectarforaging)
max(prop_gen$pollenforaging)
max(prop_gen$feedsbrood)
max(prop_gen$egglaying)

min(prop_gen$nectarforaging[prop_gen$nectarforaging > 0])
min(prop_gen$pollenforaging[prop_gen$pollenforaging > 0])
min(prop_gen$feedsbrood[prop_gen$feedsbrood > 0])
min(prop_gen$egglaying[prop_gen$egglaying > 0])
```
...
# check number of bees in count_time analyses
count_time[!is.na(count_time$shanDist),] %>% nrow()
...
ggplot(sumNests, aes(x = nestTYPE, y = nectarforaging/nBehavs)) +
  geom_point(position = position_jitter(width=0.05, height=0), alpha = 0.5) +
  stat_summary(fun = mean, geom = "point", size = 5)

ggplot(sumNests, aes(x = nestTYPE, y = pollenforaging/nBehavs)) +
  geom_point(position = position_jitter(width=0.05, height=0), alpha = 0.5) +
  stat_summary(fun = mean, geom = "point", size = 5)

ggplot(sumNests, aes(x = nestTYPE, y = feedsbrood/nBehavs)) +
  geom_point(position = position_jitter(width=0.05, height=0), alpha = 0.5) +
  stat_summary(fun = mean, geom = "point", size = 5)

ggplot(sumNests, aes(x = nestTYPE, y = egglaying/nBehavs)) +
  geom_point(position = position_jitter(width=0.05, height=0), alpha = 0.5) +
  stat_summary(fun = mean, geom = "point", size = 5)

# calculate the proportions of all behaviors carried out
# scale to nf because there are the most of that
nf <- sum(count_data$nectarforaging) / sum(count_data$nectarforaging)
pf <- sum(count_data$pollenforaging) / sum(count_data$nectarforaging)
el <- sum(count_data$egglaying) / sum(count_data$nectarforaging)
fb <- sum(count_data$feedsbrood) / sum(count_data$nectarforaging)

count_scaled <- count_data
count_scaled$pollenforaging <- count_scaled$pollenforaging / pf
count_scaled$egglaying <- count_scaled$egglaying / el
count_scaled$feedsbrood <- count_scaled$feedsbrood / fb

write.csv(count_scaled, "../Data/filtData/count_scaled.csv")

# # in order to run below code, must rerun cleanData with 0 threshold for creating count_nests 
# task_nectarforaging <- taskShan[taskShan$behav == "nectarforaging",]
# mean(task_nectarforaging$nBehav)
# sd(task_nectarforaging$nBehav)/sqrt(nrow(task_nectarforaging))
# 
# task_pollenforaging <- taskShan[taskShan$behav == "pollenforaging",]
# mean(task_pollenforaging$nBehav)
# sd(task_pollenforaging$nBehav)/sqrt(nrow(task_pollenforaging))
# 
# task_feedsbrood <- taskShan[taskShan$behav == "feedsbrood",]
# mean(task_feedsbrood$nBehav)
# sd(task_feedsbrood$nBehav)/sqrt(nrow(task_feedsbrood))
# 
# task_egglaying <- taskShan[taskShan$behav == "egglaying",]
# mean(task_egglaying$nBehav)
# sd(task_egglaying$nBehav)/sqrt(nrow(task_egglaying))
```

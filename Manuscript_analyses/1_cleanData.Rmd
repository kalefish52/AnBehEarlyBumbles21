...
ggplot(sumsTYPE, aes(x=nestTYPE, y=pollenforaging, fill=nestTYPE)) +
  geom_bar(stat="identity")+theme(axis.text.x=element_text(angle=90))
```
## calculate total duration of video watched per colony. columns = nestID and totalLen
```{r}
...
```
## graph total amount of video watched at each of the two timepoints (late and early) for each nest
```{r}

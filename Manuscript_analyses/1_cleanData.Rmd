...
```
## add dead bee info
mrege obs_data to deadReplac_data to take dead and replacement bees into account,
remove bees that died prior to observations, 
note bees that died < threshold number of days after observations
```{r}
...
```
### visualize and save duration-filtered data for analyses
write obs_data
write obs_time
write count_data
write count_time
...
## filter nest data based on number of observations
    for nest-level analyses: filter out nests below threshold number of observations each of brood feeding and foraging behaviors, includes unknown worker observations
write obs_nests
write count_nests
```{r}
...
## filter bee data based on number of observations
   for bee-level analyses: filter out bees with below threshold number of behaviors, remove unknown workers
write obs_bees
write count_beesLow
```{r}
# filter out unknown bees (these bees have NA natal colony b/c they are unidentified)
identifiedBees <- count_data[!is.na(count_data$natal),]

# filter out bees with less than threshold number of behaviors

# set threshold
obsThres <- 3
count_bees <- identifiedBees[identifiedBees$nBehavs >= obsThres,]

# make a separate df with bees with less than threshold # observations -- this includes bees with 0 observations as well
count_beesLow <- count_data[count_data$nBehavs < obsThres, ]

# count and print how many bees we have after filtering
print(paste(nrow(count_bees), "out of", nrow(count_data), "bees are identified and have at least 3 recorded observations.", sep = " "))
print(paste(nrow(count_data)-nrow(identifiedBees), "unidentified bees were filtered out of the data because they are unidentified.", sep = " "))
print(paste(nrow(identifiedBees) - nrow(count_bees), "identified bees were filtered out because they had fewer than 3 recorded observations.", sep = " "))

# save for future analyses
# count_bees manipulated in downstream analyses (shannon calculations) --> write count_bees downstream 
write.csv(count_beesLow, "../Data/filtData/count_beesLow.csv")

# filter obs_data to obs_bees for future analyses
obs_bees <- obs_data %>% subset(nestID %in% count_bees$nestID)
write.csv(obs_bees, "../Data/filtData/obs_bees.csv")
```
...
write.csv(count_nestsEarly, "../Data/filtData/count_nestsTime.csv")
```

## filter bee data at early and late timepoints
    bees with threshold number of observations at early OR late timepoints
write count_beesLowTime
```{r}
# filter out unknown bees (these bees have NA natal colony b/c they are unidentified)
identifiedBeesTime <- count_time[!is.na(count_time$natal),]

# filter out bees with less than threshold number of behaviors

# set threshold
obsThres <- 3
count_beesTime <- identifiedBeesTime[identifiedBeesTime$nBehavs >= obsThres,]
count_beesEarly <- count_beesTime[count_beesTime$timepoint == "early",]
count_beesLate <- count_beesTime[count_beesTime$timepoint == "late",]

# make a separate df with bees with less than threshold # observations -- this includes bees with 0 observations, but not unknown workers
count_beesLowTime <- identifiedBeesTime[identifiedBeesTime$nBehavs < obsThres, ]
count_beesLowEarly <- count_beesLowTime[count_beesLowTime$timepoint == "early",]
count_beesLowLate <- count_beesLowTime[count_beesLowTime$timepoint == "late",]

# count and print how many bees we have after filtering
print(paste(nrow(count_beesEarly), "out of", nrow(count_time[count_time$timepoint == "early",]), "early bees are identified and have at least 3 recorded observations.", sep = " "))
print(paste(nrow(count_time[count_time$timepoint == "early",]) - nrow(identifiedBeesTime[identifiedBeesTime$timepoint == "early",]), "unidentified early bees were filtered out because they are unidentified.", sep = " "))
print(paste(nrow(identifiedBeesTime[identifiedBeesTime$timepoint == "early",]) - nrow(count_beesEarly), "identified early bees were filtered out because they had fewer than 3 recorded observations.", sep = " "))

print(paste(nrow(count_beesLate), "out of", nrow(count_time[count_time$timepoint == "late",]), "late bees are identified and have at least 3 recorded observations.", sep = " "))
print(paste(nrow(count_time[count_time$timepoint == "late",]) - nrow(identifiedBeesTime[identifiedBeesTime$timepoint == "late",]), "unidentified late bees were filtered out because they are unidentified.", sep = " "))
print(paste(nrow(identifiedBeesTime[identifiedBeesTime$timepoint == "late",]) - nrow(count_beesLate), "identified late bees were filtered out because they had fewer than 3 recorded observations.", sep = " "))

# save for future analyses
# count_beesTime manipulated in downstream analyses (shannon calculations) --> write count_beesTime downstream 
write.csv(count_beesLowTime, "../Data/filtData/count_beesLowTime.csv")
```
...
## bee shannon
add shannon column to count_bees df
```{r}
# start with matrix where each row is an individual, and each column is a behavior
beeShan <- as.matrix(count_bees[,c("nectarforaging", "feedsbrood", "pollenforaging", "egglaying")])

# make a column to enter shannon into dol df
count_bees$shannon <- NA
# calculate shannon for each bee in the matrix and add to shannon column
count_bees$shannon <- diversity(beeShan, index = "shannon") 

# plot nestTYPE vs. shannon scatterplot
ggplot(count_bees, aes(shannon, nestTYPE, color = nestTYPE, shape = caste)) +
...
ggplot(count_bees, aes(shannon, nBehavs, color = nestTYPE, shape = caste)) +
...
add shannon and shanDist columns to count_beesTime df
```{r}
count_beesTime

# convert to matrix of behaviors to be used in shannon
dolTime <- as.matrix(count_beesTime[,c("feedsbrood", "nectarforaging", "pollenforaging")])

# calculate shannon for each bee in the matrix
count_beesTime$shannon <- diversity(dolTime, index = "shannon") 

# plot shannon values by timepoint
ggplot(count_beesTime, aes(shannon, nestTYPE, shape = interaction(caste, timepoint), fill = nestTYPE, color = nestTYPE)) + 
...
count_beesTime$shanDist <- NA

# calculate change and add to shanDist column (this will only add the value to the late timepoint row for a given bee)
for (i in 1:nrow(count_beesTime)) {                                                   # for each row in the dataset
  if(count_beesTime$timepoint[i] == "late") {                                         # look at the the late bees only
    for (j in 1:nrow(count_beesTime)) {                                               # then iterate back through the dataset
      if(count_beesTime$timepoint[j] == "early" & count_beesTime$beeID[j] == count_beesTime$beeID[i]) {   # and find the early entry with the same bee ID
        count_beesTime$shanDist[i] <- count_beesTime$shannon[i] - count_beesTime$shannon[j]               # calculate the difference in shannon index across time
      }
    }
  }
}
...
ggplot(count_beesTime, aes(shanDist, fill = nestTYPE)) + 
  geom_density(alpha = 0.3)

avgShannDist <- count_beesTime %>% dplyr::summarize(avg = mean(shanDist, na.rm = T)) %>% dplyr::pull(avg)

ggplot(count_beesTime, aes(shanDist, nestTYPE, color = nestTYPE, shape = caste)) +
...
count_beesTime[!is.na(count_beesTime$shanDist),]
```

### bee specialization
add specBehav and specProp columns to count_bees df
write count_bees
```{r}
# define shannon threshold under which represents a specialist
shannonSpecialist <- 0.6

# define number of columns before behaviors in the count_bees df
skip <- 8
# make a column for behavioral specialization
count_bees$specBehav <- NA
count_bees$specProp <- NA

for (i in 1:nrow(count_bees)) {
  # if the bee is a specialist (i.e. shannon <= threshold)
  if (count_bees$shannon[i] <= shannonSpecialist) {
    # name the behavior that each individual specialized in: column index of max number of behaviors + skip columns
    behavior <- names(count_bees[skip + max.col(count_bees[i, c("nectarforaging", "feedsbrood", "pollenforaging", "egglaying")])])
    # input that behavior into specBehav column
    count_bees$specBehav[i] <- behavior
    # calculate proportion of total behaviors comprised by specialized behavior
    count_bees$specProp[i] <- count_bees[i, behavior] / count_bees$nBehavs[i]
  }
  # if the bee is not a specialist (i.e. shannon < threshold)
  if (count_bees$shannon[i] > shannonSpecialist) {
    # name the behavior as generalist
    count_bees$specBehav[i] <- "generalist"
    # keep proportion as NA
    count_bees$specProp[i] <- NA
  }
}

# check the minimum proportion of behaviors carried out by specialists for this given shannon threshold
count_bees$specProp <- as.numeric(count_bees$specProp)
minSpecProp <- min(count_bees$specProp, na.rm = TRUE)
...
# save for future analyses
write.csv(count_bees, "../Data/filtData/count_bees.csv")
...
nSpec <- matrix(NA, ncol = 3, nrow = length(unique(count_bees$nestID))*5)
# convert to df
...
nSpec$specBehav <- c("nectarforaging", "feedsbrood", "pollenforaging", "egglaying", "generalist")
# set number of specialists to 0
nSpec$nBeesPerNest <- 0
# fill nestID with 5x each nest id (one for each behavior)
for (i in 0:(length(unique(count_bees$nestID))-1)) {
  nSpec$nestID[((i * 5) + 1):((i + 1) * 5)] <- unique(count_bees$nestID)[i + 1]
}
# replace number of bees with accurate number
# first summarize 
nSpec_missing0 <- count_bees %>% dplyr::group_by(nestID, nestTYPE, specBehav) %>% dplyr::summarise(nBeesPerNest = dplyr::n())
nSpec_missing0
...
add specBehav and specProp columns to count_beesTime df
write count_beesTime
```{r}
count_beesTime
...
skip <- 10
# make a column for behavioral specialization
count_beesTime$specBehav <- NA
count_beesTime$specProp <- NA

for (i in 1:nrow(count_beesTime)) {
  # if the bee is a specialist (i.e. shannon <= threshold)
  if (count_beesTime$shannon[i] <= shannonSpecialist) {
    # name the behavior that each individual specialized in: column index of max number of behaviors + skip columns
    behavior <- names(count_beesTime[skip + max.col(count_beesTime[i, c("nectarforaging", "feedsbrood", "pollenforaging")])])
    # input that behavior into specBehav column
    count_beesTime$specBehav[i] <- behavior
    # calculate proportion of total behaviors comprised by specialized behavior
    count_beesTime$specProp[i] <- count_beesTime[i, behavior] / count_beesTime$nBehavs[i]
  }
  # if the bee is not a specialist (i.e. shannon < threshold)
  if (count_beesTime$shannon[i] > shannonSpecialist) {
    # name the behavior as generalist
    count_beesTime$specBehav[i] <- "generalist"
    # keep proportion as NA (do nothing)
  }
}

# check the minimum proportion of behaviors carried out by specialists for this given shannon threshold
count_beesTime$specProp <- as.numeric(count_beesTime$specProp)
minSpecProp <- min(count_beesTime$specProp, na.rm = TRUE)
print(paste0("With a shannon specialist cutoff of ", shannonSpecialist, ", the specialized behavior comprises ", minSpecProp, " to 100% of all behaviors carried out by a given individual."))

# write for future analyses
write.csv(count_beesTime, "../Data/filtData/count_beesTime.csv")
...
nCat <- 4
# make empty matrix to put number into
nSpecTime <- matrix(NA, ncol = 5, nrow = length(unique(count_beesTime$nestID))*2*nCat)
# convert to df
nSpecTime <- as.data.frame(nSpecTime)
# name columns relevant info
colnames(nSpecTime) <- c("nestID", "nestTYPE", "timepoint", "specBehav", "nBeesPerNest")
# repeat the behaviors over in specBehav column
nSpecTime$specBehav <- c("nectarforaging", "feedsbrood", "pollenforaging", "generalist", "nectarforaging", "feedsbrood", "pollenforaging", "generalist")
# repeat the timepoints over in timepoint column
nSpecTime$timepoint <- c("early", "early", "early", "early", "late", "late", "late", "late")
# set number of specialists to 0
nSpecTime$nBeesPerNest <- 0
# fill nestID with nCat x each nest id (one for each behavior)
for (i in 0:(length(unique(count_beesTime$nestID))-1)) {
  nSpecTime$nestID[((i * 2 * nCat) + 1):((i + 1) * 2 * nCat)] <- unique(count_beesTime$nestID)[i + 1]
}
# replace number of bees with accurate number
# first summarize 
nSpecTime_missing0 <- count_beesTime %>% dplyr::group_by(nestID, nestTYPE, specBehav, timepoint) %>% dplyr::summarise(nBeesPerNest = dplyr::n())
nSpecTime_missing0
...
nSpecTime$specBehav <- ordered(nSpecTime$specBehav, levels = c("generalist", "pollenforaging", "nectarforaging", "feedsbrood", "egglaying"))
nSpecTime
...
write.csv(taskShan, "../Data/filtData/taskShan.csv")
...
write prop_bees
```{r}
# rename to reflect manipulations
...
for (i in 1:nrow(count_bees)) {
  count_bees$nectarforaging[i] <- count_bees$nectarforaging[i] / count_bees$nBehavs[i]
  count_bees$pollenforaging[i] <- count_bees$pollenforaging[i] / count_bees$nBehavs[i]
  count_bees$feedsbrood[i] <- count_bees$feedsbrood[i] / count_bees$nBehavs[i]
  count_bees$egglaying[i] <- count_bees$egglaying[i] / count_bees$nBehavs[i]
}
...
# for each bee, rewrite the counts as the count of a given behavior divided by total number of behaviors carried out by that bee
for (i in 1:nrow(count_beesTime)) {
  count_beesTime$nectarforaging[i] <- count_beesTime$nectarforaging[i] / count_beesTime$nBehavs[i]
  count_beesTime$pollenforaging[i] <- count_beesTime$pollenforaging[i] / count_beesTime$nBehavs[i]
  count_beesTime$feedsbrood[i] <- count_beesTime$feedsbrood[i] / count_beesTime$nBehavs[i]
}

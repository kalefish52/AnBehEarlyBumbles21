...
countsNestscale<-count_time %>% group_by(nestID) %>% 
  mutate(nestnectar=sum(nectarforaging)) %>% 
  mutate(nestpollen=sum(pollenforaging)) %>% 
  mutate(nestfeedsbrood=sum(feedsbrood)) %>% 
  #Remove nests that do not have any nectar foraging for now - 4 nests
  filter(nestnectar !=0)
...
countsNests_scale <-  countsNestscale %>% 
  mutate(nf=nestnectar/nestnectar) %>% 
  mutate(pf=nestpollen/nestnectar) %>% 
  mutate(fb=nestfeedsbrood/nestnectar)
...
#remove nest total and scaling columns 
count_scaled <- count_scaled %>% select(!c(pf, fb, nf, nestnectar, nestpollen, nestfeedsbrood)) 

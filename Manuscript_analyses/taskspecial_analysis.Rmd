...
```{r}
#read in count_data from CleanData.Rmd
...
##make a unique number vector and paste it to beeID


## takes a pattern and replaces it - gsub() 

x<-gsub("Worker[ ][0-9][ ]\\(", "",  as.character(NF.matrix$beeID)) 
x <- gsub("\\)", "", x)

x <- paste(x, 1:length(x), sep="_") #length or nrow 
...
NT.matrix <- count_data %>% filter(nestTYPE == "NT")
QF.matrix <- count_data %>% filter(nestTYPE == "QF")
QT.matrix <- count_data %>% filter(nestTYPE == "QT")


```

Try and compare different ordination al
```{r} 

...
allforaging<-dataSTATS %>% pivot_wider(names_from=behav, values_from=totCount) %>%
  select(!c( fillshoneypot, other, end, broodmanipulate, offbrood, incubate, waxmanipulate, laysegg, eatsbrood)) %>% replace(is.na(.), 0)

forage_polnec<-allforaging %>% select(!X) %>% pivot_longer(cols=c(feedsbrood, nectarforaging, pollenforaging, egglaying), names_to="behav", values_to="Count") 

combforaging<-allforaging %>% mutate(combfor=nectarforaging+pollenforaging) %>% select(!c(X, nectarforaging, pollenforaging))

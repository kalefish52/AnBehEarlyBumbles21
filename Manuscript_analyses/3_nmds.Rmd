...
specialgroups<-count_kmeans %>% dplyr::select(nestID, nestTYPE, beeID, specBehav, BehavAssign, perfSpec, cluster, NF, PF, EL, FB)
specialgroups
```
...
singleNF <- specialgroups %>% dplyr::select(nestTYPE, nestID, NF) %>% mutate(count=1) %>% group_by(nestID, NF) %>% 
  mutate(NF=sum(count)) %>% ungroup() %>% 
  dplyr::select(!count)
singlePF <- specialgroups %>% dplyr::select(nestTYPE, nestID, PF) %>% mutate(count=1) %>% group_by(nestID, PF) %>% 
  mutate(PF=sum(count)) %>% ungroup() %>% 
  dplyr::select(!count)
singleFB <- specialgroups %>% dplyr::select(nestTYPE, nestID, FB) %>% mutate(count=1) %>% group_by(nestID, FB) %>% 
  mutate(FB=sum(count)) %>% ungroup() %>% 
  dplyr::select(!count)
singleEL <- specialgroups %>% dplyr::select(nestTYPE, nestID, EL) %>% mutate(count=1) %>% group_by(nestID, EL) %>% 
  mutate(EL=sum(count)) %>% ungroup() %>% 
  dplyr::select(!count)

single <- cbind(singleNF, singlePF[,3], singleFB[,3], singleEL[,3])
single
...
single$behav <- ordered(single$behav, levels = c("FB", "NF", "PF", "EL"))

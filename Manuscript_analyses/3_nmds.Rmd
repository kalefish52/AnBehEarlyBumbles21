...
# Plot it using ggplot
ggplot() + 
  geom_point(data=nmds_data, aes(x=NMDS1, y=NMDS2, shape = cluster, color = BehavAssign, fill = specBehav), size = 3, position = position_jitter(width=0.05, height=0.05), alpha = 0.9) + 
  # scale_shape_manual(values = c(15, 17, 19, 21, 22, 24, 19)) +
  # scale_fill_manual(values = c("#F8766D", "#7CAE00", "#00BFC4", "lightgrey")) +
  # scale_color_manual(values = c("firebrick4", "darkgreen", "blue4", "gray88")) +
  scale_shape_manual(values = c(8, 21, 24)) +
  scale_fill_manual(values = c("#F8766D", "#7CAE00", "grey88", "#00BFC4")) +
  #scale_color_manual(values = c("#F8766D", "#7CAE00", "#00BFC4", "gray60")) +
  geom_text(data=behav_scores, aes(x=NMDS1, y=NMDS2, label=behav)) +
  coord_cartesian(xlim=c(-1.5, 1.5), ylim=c(-1.2, 1.1)) +
  labs(color = "highest counts", fill = "shannon-based", shape = "kmeans cluster")
...
ggplot() + 
  geom_point(data=nmds_data, aes(x=NMDS1, y=NMDS2, color = ovSavg), size = 2, position = position_jitter(width=0.05, height=0.05), alpha = 0.6) + 

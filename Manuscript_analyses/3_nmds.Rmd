...
knitr::opts_chunk$set(fig.width=3.6, fig.height=5.5) # use only to create multipanel individuals per nest plot
```
...
#single <- read.csv("../Data/filtData/single.csv")
#single <- single[single$lazy == "notLazy",]
count_kmeans
```
...
```{r}
# # we only want known bees with >=3 observations (unknown bees and bees <3 obs have NA shannon)
nmds_worker <- count_kmeans[!is.na(count_kmeans$shannon),]
# 
# # make matrix of only behavior counts for all timepoints
...
specialgroups<-count_kmeans %>% dplyr::select(nestID, nestTYPE, natal, beeID, specBehav, BehavAssign, perfSpec, cluster, NF, PF, EL, FB)
specialgroups
```
...
highcounts<-specialgroups %>% dplyr::select(nestID, natal, BehavAssign) %>% filter(BehavAssign != "NA") %>% mutate(count=1) %>% group_by(nestID, BehavAssign) %>% 
  mutate(highcounts=sum(count)) %>% ungroup() %>% 
  dplyr::select(!count)
miss<-highcounts %>% tidyr::expand(nestID, natal, BehavAssign) 
...
highcounts$BehavAssign <- ordered(highcounts$BehavAssign, levels = c("egglaying", "pollenforaging", "nectarforaging", "feedsbrood"))

G1 <- ggplot(highcounts, aes(BehavAssign, highcounts, color = nestTYPE)) + 
...
H0<-glmer(highcounts~ 1 + (1|nestTYPE:nestID), data=highcounts, family=poisson)
H1<-glmer(highcounts~ BehavAssign + (1|nestTYPE:nestID), data=highcounts, family=poisson)
H2<-glmer(highcounts~ nestTYPE + (1|nestTYPE:nestID), data=highcounts, family=poisson)
H3<-glmer(highcounts~ BehavAssign + nestTYPE + (1|nestTYPE:nestID), data=highcounts, family=poisson)
H4<-glmer(highcounts~ BehavAssign*nestTYPE + (1|nestTYPE:nestID), data=highcounts, family=poisson)
...
posthoc1 <- glht(mymodel, linfct = mcp(BehavAssign = "Tukey"))
summary(posthoc1)
...
shancounts$specBehav <- ordered(shancounts$specBehav, levels = c("egglaying", "nectarforaging", "feedsbrood", "generalist", "lazy"))

G2 <- ggplot(shancounts, aes(specBehav, shancounts, color = nestTYPE)) + 
  geom_point(position = position_jitter(width=.2, height=0.2), alpha = 0.7) +
  scale_color_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99"), labels = c("W3", "W5", "QW3", "QW5")) +
  stat_summary(fun = mean, geom = "point", size = 4, shape = 21, aes(fill = nestTYPE), color = "black") +
  scale_fill_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99")) +
...
s0<-glmer(shancounts~ 1 + (1|nestTYPE:nestID), data=shancounts, family=poisson)
s1<-glmer(shancounts~ specBehav + (1|nestTYPE:nestID), data=shancounts, family=poisson)
s2<-glmer(shancounts~ nestTYPE + (1|nestTYPE:nestID), data=shancounts, family=poisson)
s3<-glmer(shancounts~ specBehav + nestTYPE + (1|nestTYPE:nestID), data=shancounts, family=poisson)
s4<-glmer(shancounts~ specBehav*nestTYPE + (1|nestTYPE:nestID), data=shancounts, family=poisson)
...
perfcounts<-specialgroups %>% dplyr::select(nestID, natal, BehavAssign, perfSpec) %>% filter(perfSpec != "NA") %>% mutate(count=1) %>% group_by(nestID, BehavAssign) %>% 
  mutate(perfcounts=sum(count)) %>% ungroup() %>% 
  dplyr::select(!count)
miss<-perfcounts %>% tidyr::expand(nestID, natal, BehavAssign) 
...
perfcounts$BehavAssign <- ordered(perfcounts$BehavAssign, levels = c("egglaying", "nectarforaging", "feedsbrood"))

G3 <- ggplot(perfcounts, aes(BehavAssign, perfcounts, color = nestTYPE)) + 
  geom_point(position = position_jitter(width=.2, height=0.2), alpha = 0.7) +
  scale_color_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99"), labels = c("W3", "W5", "QW3", "QW5")) +
  stat_summary(fun = mean, geom = "point", size = 4, shape = 21, aes(fill = nestTYPE), color = "black") +
  scale_fill_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99")) +
...
head(perfcounts)
p0<-glmer(perfcounts~ 1  + (1|nestTYPE:nestID), data=perfcounts, family=poisson)
p1<-glmer(perfcounts~ BehavAssign + (1|nestTYPE:nestID), data=perfcounts, family=poisson)
p2<-glmer(perfcounts~ nestTYPE + (1|nestTYPE:nestID), data=perfcounts, family=poisson)
p3<-glmer(perfcounts~ BehavAssign + nestTYPE  + (1|nestTYPE:nestID), data=perfcounts, family=poisson)
p4<-glmer(perfcounts~ BehavAssign*nestTYPE  + (1|nestTYPE:nestID), data=perfcounts, family=poisson)
...
kcounts<-specialgroups %>% dplyr::select(nestID, natal, cluster) %>% filter(cluster != "NA") %>% mutate(count=1) %>% group_by(nestID, cluster) %>% 
  mutate(kcounts=sum(count)) %>% ungroup() %>% 
  dplyr::select(!count)
miss<-kcounts %>% tidyr::expand(nestID, natal, cluster) 
...
G4 <- ggplot(kcounts, aes(cluster, kcounts, color = nestTYPE)) + 
  geom_point(position = position_jitter(width=.2, height=0.2), alpha = 0.7) +
  scale_color_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99"), labels = c("W3", "W5", "QW3", "QW5")) +
  stat_summary(fun = mean, geom = "point", size = 4, shape = 21, aes(fill = nestTYPE), color = "black") +
  scale_fill_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99")) +
...
head(kcounts)
k0<-glmer(kcounts~ 1 + (1|natal) + (1|nestTYPE:nestID), data=kcounts, family=poisson)
k1<-glmer(kcounts~ cluster + (1|natal) + (1|nestTYPE:nestID), data=kcounts, family=poisson)
k2<-glmer(kcounts~ nestTYPE + (1|natal) + (1|nestTYPE:nestID), data=kcounts, family=poisson)
k3<-glmer(kcounts~ cluster + nestTYPE + (1|natal) + (1|nestTYPE:nestID), data=kcounts, family=poisson)
k4<-glmer(kcounts~ cluster*nestTYPE + (1|natal) + (1|nestTYPE:nestID), data=kcounts, family=poisson)
...
```{r}
head(specialgroups)
# count number of nests with a bee that carried out each behavior
specialgroups %>% filter(NF=="yes") %>% dplyr::select(nestID) %>% unique() %>% nrow()
specialgroups %>% filter(PF=="yes") %>% dplyr::select(nestID) %>% unique() %>% nrow()
specialgroups %>% filter(FB=="yes") %>% dplyr::select(nestID) %>% unique() %>% nrow()
specialgroups %>% filter(EL=="yes") %>% dplyr::select(nestID) %>% unique() %>% nrow()

# count number of bees that perform each behavior in each nest
singleNF <- specialgroups %>% dplyr::select(nestTYPE, natal, nestID, NF) %>% mutate(count=1) %>% group_by(nestID, NF) %>% 
  mutate(NF=sum(count)) %>% ungroup() %>% 
  dplyr::select(!count)
singlePF <- specialgroups %>% dplyr::select(nestTYPE, natal, nestID, PF) %>% mutate(count=1) %>% group_by(nestID, PF) %>% 
  mutate(PF=sum(count)) %>% ungroup() %>% 
  dplyr::select(!count)
singleFB <- specialgroups %>% dplyr::select(nestTYPE, natal, nestID, FB) %>% mutate(count=1) %>% group_by(nestID, FB) %>% 
  mutate(FB=sum(count)) %>% ungroup() %>% 
  dplyr::select(!count)
singleEL <- specialgroups %>% dplyr::select(nestTYPE, natal, nestID, EL) %>% mutate(count=1) %>% group_by(nestID, EL) %>% 
  mutate(EL=sum(count)) %>% ungroup() %>% 
  dplyr::select(!count)

singles <- cbind(singleNF, singlePF[,4], singleFB[,4], singleEL[,4])
singles

# gather behaviors into a single column for graphing
single <- singles %>%
  gather(NF, PF, FB, EL, 
         key = "behav", 
         value = "instance")
...
# order nesttype as a factor for graphing consistency
single$nestTYPE <- ordered(single$nestTYPE, levels = c("NT", "NF", "QT", "QF"))
single$behav <- ordered(single$behav, levels = c("EL", "PF", "NF", "FB"))

# plot number of individuals carrying out each behavior in each nest
G5 <- ggplot(single, aes(behav, instance, color = nestTYPE)) + 
  geom_point(position = position_jitter(width=.2, height=0.2), alpha = 0.7) +
  scale_color_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99"), labels = c("W3", "W5", "QW3", "QW5")) +
  stat_summary(fun = mean, geom = "point", size = 4, shape = 21, aes(fill = nestTYPE), color = "black") +
  scale_fill_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99")) +
  ggtitle("Tasks performed at least once") +
  scale_x_discrete(labels=c("NF" = "Nectar\ncollection", "PF" = "Pollen\ncollection", "FB" = "Brood\nfeeding", "EL" = "Egg\nlaying")) +
  labs(color="Social configuration", y = "Number of individuals per nest", x = "Task") +
  guides(fill=FALSE, color = guide_legend(override.aes = list(size = 5)))  
G5
...
k0<-glmer(instance ~ 1 + (1|nestTYPE:nestID), data=single, family=poisson)
k1<-glmer(instance~ behav + (1|nestTYPE:nestID), data=single, family=poisson)
k2<-glmer(instance~ nestTYPE + (1|nestTYPE:nestID), data=single, family=poisson)
k3<-glmer(instance~ behav + nestTYPE + (1|nestTYPE:nestID), data=single, family=poisson)
k4<-glmer(instance~ behav*nestTYPE + (1|nestTYPE:nestID), data=single, family=poisson)

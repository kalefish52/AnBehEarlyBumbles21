...
p2 <- ggplot() + 
  ...
  scale_fill_manual(values = alpha(c("#F8766D", "#7CAE00", "#00BFC4", "white"), 0.4), 
...
specialgroups<-count_kmeans %>% dplyr::select(nestID, nestTYPE, specBehav, BehavAssign, perfSpec, cluster)
head(specialgroups)

#How many specialists in highest counts? 
highcounts<-specialgroups %>% dplyr::select(nestID, nestTYPE, 
                                            BehavAssign) %>% 
  filter(BehavAssign != "NA") %>% mutate(count=1) %>% group_by(nestID, BehavAssign) %>% 
  mutate(highcounts=sum(count)) %>% ungroup() %>% 
  group_by(nestTYPE, BehavAssign) %>% 
  mutate(highcountsavg=mean(highcounts)) %>% ungroup() %>% dplyr::select(!count) %>% unique()
head(highcounts)
...
G1<-ggplot(highcounts, aes(BehavAssign, highcounts, color = nestTYPE)) + geom_point(position = position_jitter(width=0.05, height=0.2), alpha = 0.6, aes(color = nestTYPE)) +
  ...
  ylab("Number of Individuals") + xlab("Behavior") + scale_x_discrete(labels=c("egglaying" = "Egg\n Laying", "feedsbrood" = "Brood\n Feeding", "nectarforaging" = "Nectar\n Collection", "pollenforaging" = "Pollen\n Collection")) 
G1 

#How many specialists in shannon? 
shancounts<-specialgroups %>% dplyr::select(nestID, nestTYPE, 
                                            specBehav) %>% filter(specBehav != "NA") %>% mutate(count=1) %>% group_by(nestID, specBehav) %>% 
  mutate(shancounts=sum(count)) %>% ungroup() %>% 
  group_by(nestTYPE, specBehav) %>% 
  mutate(shancountsavg=mean(shancounts)) %>% ungroup() %>% dplyr::select(!count) %>% unique()
...
perfcounts<-specialgroups %>% dplyr::select(nestID, nestTYPE, 
                                            perfSpec, BehavAssign) %>% filter(perfSpec != "NA") %>% mutate(count=1) %>% group_by(nestID, BehavAssign) %>% 
  mutate(perfcounts=sum(count)) %>% ungroup() %>% 
  group_by(nestTYPE, BehavAssign) %>% 
  mutate(perfcountsavg=mean(perfcounts)) %>% ungroup() %>% dplyr::select(!count) %>% unique()
perfcounts
...
G3<-ggplot(perfcounts, aes(BehavAssign, perfcounts, color = nestTYPE)) + geom_point(position = position_jitter(width=0.05, height=0.2), alpha = 0.6, aes(color = nestTYPE)) +
  ...
  ylab("Number of Individuals") + xlab("Behavior") 
G3

# How many kmeans clusters? 
specialgroups$cluster<-as.factor(specialgroups$cluster)
kcounts<-specialgroups %>% dplyr::select(nestID, nestTYPE, 
                                            cluster) %>% filter(cluster != "NA") %>% mutate(count=1) %>% group_by(nestID, cluster) %>% 
  mutate(kcounts=sum(count)) %>% ungroup() %>% 
  group_by(nestTYPE, cluster) %>% 
  mutate(kcountsavg=mean(kcounts)) %>% ungroup() %>% dplyr::select(!count) %>% unique()

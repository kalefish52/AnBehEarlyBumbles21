...
# load data
```{r}
early_data <- read.csv("../Data/tempData/beeEarlyCounts_filt.csv")
late_data <- read.csv("../Data/tempData/beeLateCounts_filt.csv")
...
nmds_data <-nmds_data[rowSums(nmds_data[,c("feedsbrood", "nectarforaging", "pollenforaging")]) > 0, ]
...
ggplot() + 
  geom_point(data=data.scores, aes(x=NMDS1, y=NMDS2, shape=caste, color = timepoint), size = 2, position = position_jitter(width=0.08, height=0.08), alpha = 0.7) + 
...
p <- ggplot() + 
  ...
  geom_point(data=data.scores, aes(x=NMDS1, y=NMDS2, shape=caste, color = timepoint), position = position_jitter(width=0.05, height=0.05)) + 
  scale_shape_manual(values = c(21, 16)) +
...
p <- ggplot() + 
  ...
  geom_point(data=data.scores, aes(x=NMDS1, y=NMDS2, shape=caste, color = timepoint), size = 2, position = position_jitter(width=0.05, height=0.05)) + 
    scale_shape_manual(values = c(21, 16)) +
  # geom_line(data=data.scores, aes(x=NMDS1, NMDS2, fill = beeID), size = 0.15,
  #          arrow = arrow(length=unit(0.15,"cm"), type = "open")) +
  coord_cartesian(xlim=c(-2, 2), ylim=c(-2, 2))
...
<!-- fitBinom <- fitdistr(data=nmdsData$NMDS1, dist="binom") -->
<!-- qqp(nmdsData$NMDS1, "binom", size=100, fitBinom$estimate) -->
...
<!-- library(lme4) -->
<!-- nmds1 <- glmer(NMDS1 ~ timepoint*nestTYPE + timepoint*caste + (1|beeID) + (1|nestID), -->
<!--               data = nmdsData) -->

<!-- modelResiduals <- resid(nmds1, type = "pearson") # Extract standardized residuals -->
<!-- hist(modelResiduals) -->
<!-- qqnorm(resid(nmds1)) -->
<!-- plot(nmds1) -->
...
<!-- library(lme4) -->
<!-- nmdsData$NMDS2.s <- sqrt(nmdsData$NMDS2 + 2) -->
<!-- hist(nmdsData$NMDS2) -->

<!-- # poisson must be positive whole numbers -->
<!-- poisson <- fitdistr(nmdsData$NMDS2, "Poisson") -->
<!-- qqp(nmdsData$NMDS2, "pois", lambda = poisson$estimate) -->

<!-- nmds2 <- glmer(NMDS2 ~ timepoint*nestTYPE + timepoint*caste + (1|beeID) + (1|nestID), -->
<!--               data = nmdsData) -->
<!-- qqnorm(resid(nmds2)) -->
<!-- hist(resid(nmds2)) -->
...
# NMDS1: low = brood feeding, high = foraging
...
# NMDS2: low = brood feeding + nectar foraging, high = pollen foraging

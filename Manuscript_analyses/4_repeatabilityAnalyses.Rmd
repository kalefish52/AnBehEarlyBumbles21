...
# load data
```{r}
rptdata<-read.csv("../Data/filtData/prop_beesTime.csv") %>% select(nestID, nestTYPE, beeID, caste, timepoint, nectarforaging, feedsbrood, pollenforaging)
head(rptdata)
obsdata<-read.csv("../Data/filtData/obs_time.csv") %>% select(beeID, camID)

#How many observations (nest and for) did we do for each beeID? 
obsdatascale<-obsdata %>% mutate(count=1) %>% group_by(beeID, camID) 
obsdatascale %>% mutate(obsnum=sum(count))

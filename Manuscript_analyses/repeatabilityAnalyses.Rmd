...
# load data
```{r}
initial_data <- read.csv("../Data/tempData/beeCounts_filt.csv")
initial_data
```

# NMDS on beeProp data
...
# plot it split out nestID
early.scores <- data.scores[data.scores$timepoint == "early", ]
late.scores <- data.scores[data.scores$timepoint == "late", ]

p <- ggplot() +
  geom_text(data=behav.scores, aes(x=NMDS1, y=NMDS2, label=behav), size = 2) +
  geom_point(data=early.scores, aes(x=NMDS1, y=NMDS2), color = "red") + 
  coord_cartesian(xlim=c(-2, 2), ylim=c(-2, 2))
earlyPlots = early.scores %>%
    do(plots = p %+% . + facet_wrap(~nestID))
earlyPlots$plots
ggsave("../figures/nmds_early_SPLITbyNESTID.pdf")

p <- ggplot() +
  geom_text(data=behav.scores, aes(x=NMDS1, y=NMDS2, label=behav), size = 2) +
  geom_point(data=late.scores, aes(x=NMDS1, y=NMDS2), color = "red") + 
  coord_cartesian(xlim=c(-2, 2), ylim=c(-2, 2))
latePlots = early.scores %>%
    do(plots = p %+% . + facet_wrap(~nestID))
latePlots$plots
ggsave("../figures/nmds_late_SPLITbyNESTID.pdf")

p <- ggplot() + 
  geom_text(data=behav.scores, aes(x=NMDS1, y=NMDS2, label=behav), size = 2) +
  geom_point(data=data.scores, aes(x=NMDS1, y=NMDS2, color=timepoint, shape=caste), position = position_jitter(width=0.05, height=0.05)) + 
...
plots = data.scores %>%
    do(plots = p %+% . + facet_wrap(~nestID))
plots$plots
ggsave("../figures/nmds_all_SPLITbyNESTID.pdf")

# plot split out by nestTYPE
p <- ggplot() + 
  geom_text(data=behav.scores, aes(x=NMDS1, y=NMDS2, label=behav), size = 3) +
  geom_point(data=data.scores, aes(x=NMDS1, y=NMDS2, color=timepoint, shape=caste), size = 2, position = position_jitter(width=0.05, height=0.05)) + 
  scale_color_manual(values = c("red", "blue")) +
  # geom_line(data=data.scores, aes(x=NMDS1, NMDS2, fill = beeID), size = 0.15,
  #          arrow = arrow(length=unit(0.15,"cm"), type = "open")) +
  coord_cartesian(xlim=c(-2, 2), ylim=c(-2, 2))
...
plots$plots
ggsave("../figures/nmds_all_SPLITbyNESTtype.pdf")
```
  ...
  if(data.scores$timepoint[i] == "late") {                                                      # look at the the late bees only
    for (j in 1:nrow(data.scores)) {                                                            # then iterate back through the dataset
      if(data.scores$timepoint[j] == "early" & data.scores$beeID[j] == data.scores$beeID[i]) {  # and find the early entry with the same bee ID 
        ...
        totalDistance <- sqrt( (x2 - x2)^2 + (y2 - y1)^2 )                                      # formula for distance between two points
        data.scores$totalDist[i] <- totalDistance                                               # fill in the dataframe with distances between early and late
        data.scores$nmds1dist[i] <- data.scores$NMDS1[i] - data.scores$NMDS1[j]
        data.scores$nmds2dist[i] <- data.scores$NMDS2[i] - data.scores$NMDS2[j]
...
# summarize change in NMDS1
distNMDS1 <- ddply(data.scores, c("nestTYPE", "caste"), 
                summarise,
                mean = mean(NMDS1, na.rm = TRUE), 
                sd = sd(NMDS1, na.rm = TRUE),
                sem = sd(NMDS1, na.rm = TRUE)/sqrt(length(!is.na(NMDS1))))
distNMDS1

# summarize change in NMDS2
distNMDS2 <- ddply(data.scores, c("nestTYPE", "caste"), 
                summarise,
                mean = mean(NMDS2, na.rm = TRUE), 
                sd = sd(NMDS2, na.rm = TRUE),
                sem = sd(NMDS2, na.rm = TRUE)/sqrt(length(!is.na(NMDS2))))
distNMDS2
```

...
# nest data
nest_data <- read.csv("../../Data/rawData/NestDissections.csv", header = TRUE)

# wing length data
wing_data <- read.csv("../../Data/rawData/MaleWingMeasurements.csv", header = TRUE)

# development times data
dev_data <- read.csv("../Data/rawData/cleanMasterData.csv")

# count_data has natal colony info and filtered nests
...
```{r}
# remove bad data
wing_data <- wing_data %>% filter(nestID %in% count_data$nestID)
nest_data <- nest_data %>% filter(nestID %in% count_data$nestID)

# add male bee identifier to wing_data
...
# add worker natal colony to include as random effect

# add queen natal colony to include as random effect


# qChar <- sapply(strsplit(dev_data$nestID, ""), function(x) {x[1]} )
# wChar <- sapply(strsplit(dev_data$nestID, ""), function(x) {x[2]} )
# dev_data$nestTYPE <- paste0(qChar, wChar)
# dev_data$nWorkers <- wChar
# dev_data$nWorkers[dev_data$nWorkers == "F"] <- 5
# dev_data$nWorkers[dev_data$nWorkers == "T"] <- 3
# dev_data$nQueen <- qChar
# dev_data$nQueen[dev_data$nQueen == "N"] <- 0
# dev_data$nQueen[dev_data$nQueen == "Q"] <- 1
# dev_data$nWorkers <- as.numeric(dev_data$nWorkers)
# dev_data$nQueen <- as.numeric(dev_data$nQueen)
# dev_data$nBees <- dev_data$nWorkers + dev_data$nQueen
# dev_data
...
sum_brood <- ddply(nest_data, 
                c("nestTYPE"), 
                summarise,
                mean = mean(nBrood, na.rm = TRUE), 
                sd = sd(nBrood, na.rm = TRUE),
                sem = sd(nBrood, na.rm = TRUE)/sqrt(length(nBrood)))
sum_brood

sum_size <- ddply(wing_data, 
                c("nestTYPE"), 
                summarise,
                mean = mean(beeAvg, na.rm = TRUE), 
                sd = sd(beeAvg, na.rm = TRUE),
                sem = sd(beeAvg, na.rm = TRUE)/sqrt(length(beeAvg)))
sum_size

sum_pupae <- ddply(nest_data, 
                c("nestTYPE"), 
                summarise,
                mean = mean(pupae, na.rm = TRUE), 
                sd = sd(pupae, na.rm = TRUE),
                sem = sd(pupae, na.rm = TRUE)/sqrt(length(pupae)))
sum_pupae

sum_larvae <- ddply(nest_data, 
                c("nestTYPE"), 
                summarise,
                mean = mean(totLarvae, na.rm = TRUE), 
                sd = sd(totLarvae, na.rm = TRUE),
                sem = sd(totLarvae, na.rm = TRUE)/sqrt(length(totLarvae)))
sum_larvae

sum_eggs <- ddply(nest_data, 
                c("nestTYPE"), 
                summarise,
                mean = mean(eggs, na.rm = TRUE), 
                sd = sd(eggs, na.rm = TRUE),
                sem = sd(eggs, na.rm = TRUE)/sqrt(length(eggs)))
sum_eggs
```

# mixed models
```{r}
# response variables = # brood total, # brood split out by type, male size, # days to lay eggs, # days eggs to eclosion
...
### number of days to lay first eggs: NS
...
# define model
dayEggs1 <- lm(DaysToEggs ~ nestTYPE, data = dev_data)
dayEggs2 <- lm(DaysToEggs ~ nQueen + nBees, data = dev_data)
...
### number of days from eggs to eclosion: NS 
data is approximately normal
```{r}
# visualize the distribution
...
# define model
dayEclos1 <- lm(EggsToEclosion ~ nestTYPE, data = dev_data)
dayEclos2 <- lm(EggsToEclosion ~ nQueen + nBees, data = dev_data)
...
### number of days from nest start to eclosion: nestTYPE p < 0.01, nBees < 0.01 
data is approximately normal
```{r}
# visualize the distribution
ggdensity(dev_data$TotalNestTime)
# normal 
qqp(dev_data$TotalNestTime, "norm")
# # lnorm means lognormal
# qqp(dev_data$TotalNestTime, "lnorm")
# # gamma must be positive numbers
# gamma <- fitdistr(dev_data$TotalNestTime, "gamma")
# qqp(dev_data$TotalNestTime, "gamma", shape = gamma$estimate[[1]], rate = gamma$estimate[[2]])

# define model
dayNest1 <- lm(TotalNestTime ~ nestTYPE, data = dev_data)
dayNest2 <- lm(TotalNestTime ~ nQueen + nBees, data = dev_data)

---
title: "kMeans"
output: pdf_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

# Load Packages
```{r}
library(tidyverse)
library(plotly)
library(dplyr)
library(multcomp)
```

## Load Data
```{r}
# pooled data across timepoints, values are counts
# includes all bees, but only known bees with >=3 observations have shannon values
count_data <- read.csv("../Data/filtData/count_data.csv")

# remove unknown bees
count_data <- count_data[!grepl("unknown", count_data$beeID), ]

```

# K Means Cluster Analyses -- determine whether points cluster into two or more groups
Elbow Method for finding the optimal number of clusters. optimal number of clusters = elbow of the graph. (in this case 3)
```{r find number of clusters in k means}
## set seed of random number generator for reproducibility
set.seed(123)

## compute and plot total within sum of squares for 2-10 clusters
k.max <- 10
## make sure the subset of count_data here includes feedsbrood, nectarforage, pollenforage, egglaying
data <- count_data[, c("feedsbrood", "nectarforaging", "pollenforaging", "egglaying")]

wss <- sapply(1:k.max, 
              function(k){kmeans(data, k, nstart=50, iter.max = 20)$tot.withinss})
wss
## plot and look to find the elbow. here it is at 3
elbow <- plot(1:k.max, wss,
     type="b", pch = 19, frame = FALSE, 
     xlab="Number of clusters K",
     ylab="Total within-clusters sum of squares")
```

run k means analysis
```{r}
## set random number generator seed for reproducibility
set.seed(124)

## set k based on where elbow is in above graph
k <- 2

## run k means test
test <- kmeans(data, k, nstart = 20)
length(as.factor(test$cluster))
```
 graph k means analysis
```{r} 
## make cluster a factor so we can graph with it
count_data$cluster <- as.factor(test$cluster)

## export as count_kmeans
write.csv(count_data, "../Data/filtData/count_kmeans.csv")

## graph the data in 3d. color code based on cluster, make queens solid circles and workers open circles. 

## make egg laying workers x 
plot_ly(x = count_data$feedsbrood, 
        y = count_data$allforaging, 
        z = count_data$egglaying, 
        type="scatter3d", 
        mode="markers",
        symbol = count_data$caste, symbols = c('square', 'circle'),
        color = count_data$cluster) %>%
  layout(
    title = "k means clustering",
    scene = list(
      xaxis = list(title = "feedsbrood"),
      yaxis = list(title = "allforaging"),
      zaxis = list(title = "egglaying"))
  )

#Plot counts of each cluster for each specBehav
count_data
kcounts<-count_data %>% dplyr::select(nestID, beeID, specBehav, cluster)

head(kcounts)

kSUM<-kcounts %>% group_by(specBehav, cluster) %>% mutate(count=1) %>% mutate(frequency=sum(count)) %>% dplyr::select(specBehav, cluster, frequency) %>% unique()

ggplot(kSUM, aes(x=specBehav, y=frequency, fill=cluster)) +
    geom_bar(stat="identity", position=position_dodge(), colour="black")

ggplot(count_data, aes(x = allforaging, y = cluster)) +
  geom_point()
ggplot(count_data, aes(x = nectarforaging, y = cluster)) +
  geom_point()
ggplot(count_data, aes(x = pollenforaging, y = cluster)) +
  geom_point()
ggplot(count_data, aes(x = feedsbrood, y = cluster)) +
  geom_point()
ggplot(count_data, aes(x = egglaying, y = cluster)) +
  geom_point()
min(count_data[count_data$cluster == 2, "nectarforaging"])
max(count_data[count_data$cluster == 1, "nectarforaging"])

```
# Do groups correspond to ovary/body size data?  
```{r}
kmeanstrends<-count_data %>% select(size, ovS1, ovS2, cluster) %>%mutate(avgov=(ovS1+ovS2)/2) %>% na.omit()
count_data
count_data$avgOvS <- NA
for (i in 1:nrow(count_data)) {
  count_data$avgOvS[i] <- (count_data$ovS1[i] + count_data$ovS2[i]) / 2
}

graphkmeans<-kmeanstrends %>% group_by(cluster) %>% 
  mutate(asize=mean(size)) %>%  
  mutate(sdsize=sd(size)) %>% 
  mutate(aov=mean(avgov))  %>% 
  mutate(sdov=sd(avgov)) %>% 
  select(cluster, asize, sdsize, aov, sdov) %>% unique()

BSk<-ggplot(graphkmeans, aes(x=cluster, y=asize)) +
    geom_bar(stat="identity", position=position_dodge(), colour="black") + 
  geom_errorbar(aes(ymin=asize-sdsize,  ymax=asize+sdsize))
BSk

OVk<-ggplot(graphkmeans, aes(x=cluster, y=aov)) +
    geom_bar(stat="identity", position=position_dodge(), colour="black") + 
  geom_errorbar(aes(ymin=aov-sdov,  ymax=aov+sdov))
OVk

library(lme4)
library(sjPlot)  # tab_model
library(MuMIn)    # model.sel

count_data$cluster <- as.factor(count_data$cluster)

k0 <- glmer(cluster ~ 1 + (1|nestTYPE) + (1|natal),
            family = binomial,
            data = count_data)
k1 <- glmer(cluster ~ avgOvS + (1|nestTYPE) + (1|natal),
            family = binomial,
            data = count_data)
k2 <- glmer(cluster ~ size + (1|nestTYPE) + (1|natal) + (1|caste),
            family = binomial,
            data = count_data)


tab_model(k1)
tab_model(k2)

library(gridExtra)
p1 <- ggplot(count_data, aes(cluster, size, shape = caste, color = cluster)) +
  stat_summary(fun = mean, geom = "point", size = 5) +
  geom_point(position = position_jitter(width=0.1, height=0.05), alpha = 0.5) +
  scale_shape_manual(values = c(21, 16)) +
  scale_color_manual(values = c("aquamarine3", "darkslateblue"))
ggsave("../figures/kmeansBS.jpg")
p1

p2 <- ggplot(count_data, aes(cluster, avgOvS, shape = caste, color = cluster)) +
  stat_summary(fun = mean, geom = "point", size = 5) +
  geom_point(position = position_jitter(width=0.1, height=0.05), alpha = 0.5) +
  scale_shape_manual(values = c(21, 16)) + 
  scale_color_manual(values = c("aquamarine3", "darkslateblue"))
p2
ggsave("../figures/kmeansOv.jpg")

ggarrange(p1, p2, labels = c("A", "B"))
ggsave("../figures/kmeansBSov.jpg")
```


# Do analysis based on highest number of tasks workers perform 
```{r}
head(count_data)
highbehav<-count_data %>% select(beeID, nectarforaging, 
                                 feedsbrood, pollenforaging, egglaying)

hbdf<-highbehav %>% pivot_longer(!beeID, names_to="Behavior", values_to="count")

#Sort each bee by which behavior they do the most and then assign that bee to that behavior
BehavAssign<-hbdf %>% group_by(beeID) %>% arrange(beeID, desc(count)) %>% slice_head() %>% mutate(BehavAssign=Behavior) %>% select(beeID, BehavAssign)
BehavAssign

#Merge back into original dataframe
FinalBehavRank<-left_join(count_data, BehavAssign, by="beeID")
FinalBehavRank$BehavAssign <-as.factor(FinalBehavRank$BehavAssign)

ggplot(FinalBehavRank, aes(BehavAssign)) + geom_bar()
ggsave("../figures/highcountdistribution.jpg")


#Run GLMMs using BehavAssign as a grouping variable 
FinalBehavRank
ggplot(FinalBehavRank, aes(BehavAssign, size, color = caste, shape = nestTYPE)) +
  geom_point(position = position_jitter(width=0.1, height=0.05), alpha = 0.5) +
  stat_summary(fun = mean, geom = "point", size = 5)
ggsave("../figures/highcountBS.jpg")

hist(FinalBehavRank$size)
j0 <- glmer(size~ 1 + (1|nestTYPE) + (1|natal),
            family = gaussian,
            data = FinalBehavRank)
j1 <- glmer(size ~ BehavAssign + (1|nestTYPE) + (1|natal) + (1|caste),
            family = gaussian,
            data = FinalBehavRank)
tab_model(j1)

posthoc1 <- glht(j1, linfct = mcp(BehavAssign = "Tukey"))
summary(posthoc1)


```




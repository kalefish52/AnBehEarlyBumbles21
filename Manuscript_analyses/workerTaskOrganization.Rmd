...
# # multipanel shannon value plot
# knitr::opts_chunk$set(fig.width=2.5, fig.height=4.5)

# # multipanel single task frameworks
...
# # multipanel change in shannon value plot
# knitr::opts_chunk$set(fig.width=3, fig.height=3.5) 
...
# load data
```{r}
# pooled data across timepoints, values are scaled counts
...
# 1. SHANNON VALUE
use count_data (include workers with <3 obs as shannon = NA)
...
# how many individuals are perfectly speciailzed?
nrow(count_3[count_3$shannon == 0, ])
...
2. SINGLE TASK FRAMEWORKS

# 2.1 MOST FREQUENTLY PERFORMED TASK
use count_3 (exclude workers with <3 obs)
...
```{r}
# How many nests total?
length(unique(FinalBehavRank$nestID))

# How many bees are in each category? 
paste(nrow(FinalBehavRank[FinalBehavRank$BehavAssign == "nectarforaging",]), "highest counts nectar collectors from ", length(unique(FinalBehavRank$nestID[FinalBehavRank$BehavAssign == "nectarforaging"])), "nests")

paste(nrow(FinalBehavRank[FinalBehavRank$BehavAssign == "pollenforaging",]), "highest counts pollen collectors from ", length(unique(FinalBehavRank$nestID[FinalBehavRank$BehavAssign == "pollenforaging"])), "nests")

paste(nrow(FinalBehavRank[FinalBehavRank$BehavAssign == "feedsbrood",]), "highest counts brood feeders from ", length(unique(FinalBehavRank$nestID[FinalBehavRank$BehavAssign == "feedsbrood"])), "nests")

paste(nrow(FinalBehavRank[FinalBehavRank$BehavAssign == "egglaying",]), "highest counts egg layers from ", length(unique(FinalBehavRank$nestID[FinalBehavRank$BehavAssign == "egglaying"])), "nests")
...
highcounts$nestTYPE <- as.character(highcounts$nestTYPE)
...
highcounts_nestSum$nestTYPE <- ordered(highcounts_nestSum$nestTYPE, levels = c("NT", "QT", "NF", "QF"))
...
paste(nrow(perfSpec), "perfectly specialized workers total from ", length(unique(perfSpec$nestID)), " nests")
...
paste(nrow(perfSpec[perfSpec$specBehav == "nectarforaging",]), "perfectly specialized nectar collectors from ", length(unique(perfSpec$nestID[perfSpec$specBehav == "nectarforaging"])), "nests")

paste(nrow(perfSpec[perfSpec$specBehav == "pollenforaging",]), "perfectly specialized pollen collectors ", length(unique(perfSpec$nestID[perfSpec$specBehav == "pollenforaging"])), "nests")

paste(nrow(perfSpec[perfSpec$specBehav == "feedsbrood",]), "perfectly specialized brood feeders ", length(unique(perfSpec$nestID[perfSpec$specBehav == "feedsbrood"])), "nests")

paste(nrow(perfSpec[perfSpec$specBehav == "egglaying",]), "perfectly specialized egg layers ", length(unique(perfSpec$nestID[perfSpec$specBehav == "egglaying"])), "nests")
...
perfSpec$nestTYPE <- as.character(perfSpec$nestTYPE)
...
perfSpec_nestSum$nestTYPE <- ordered(perfSpec_nestSum$nestTYPE, levels = c("NT", "QT", "NF", "QF"))
...
single$nestTYPE <- as.character(single$nestTYPE)
...
single_nestSum$nestTYPE <- ordered(single_nestSum$nestTYPE, levels = c("NT", "QT", "NF", "QF"))
...
G5 <- ggplot(single_nestSum, aes(behav, number, color = nestTYPE)) + 
  ...
  stat_summary(fun = mean, geom = "point", size = 4, shape = 21, aes(fill = nestTYPE), color = "black") +
  scale_fill_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99")) +
  ggtitle("Task performed at least once") +
  scale_x_discrete(labels=c("el" = "Egg\nlaying", "bf" = "Brood\nfeeding", "nf" = "Nectar\ncollection", "pf" = "Pollen\ncollection")) +
  labs(color="Social configuration", y = "Number of individuals per nest", x = "Task") +
  guides(fill=FALSE, color = guide_legend(override.aes = list(size = 5)))
G5
...
3. MULTI-TASK FRAMEWORKS

# 3.1 KMEANS
use count_data (include workers with <3 obs)
...
summary(k3)
posthoc1 <- glht(k3, linfct = mcp(cluster = "Tukey"))
summary(posthoc1)
...
```{r}
paste(nrow(count_data[count_data$cluster == "1",]), "kmeans cluster 1 from", length(unique(count_data$nestID[count_data$cluster == "1"])), "nests")

paste(nrow(count_data[count_data$cluster == "2",]), "kmeans cluster 2 from", length(unique(count_data$nestID[count_data$cluster == "2"])), "nests")
...
kdata$nestTYPE <- as.character(kdata$nestTYPE)
...
kdata_nestSum$nestTYPE <- ordered(kdata_nestSum$nestTYPE, levels = c("NT", "QT", "NF", "QF"))

G4 <- ggplot(kdata_nestSum, aes(x = cluster, y = number, color = nestTYPE)) + 
  geom_point(position = position_jitter(width=.2, height=0.2), alpha = 0.7) +
  scale_color_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99"), labels = c("W3", "W5", "QW3", "QW5")) +
  stat_summary(fun = mean, geom = "point", size = 4, shape = 21, aes(fill = nestTYPE), color = "black") +
  scale_fill_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99")) +
  ggtitle("K-means categorization") +
  scale_x_discrete(labels=c("one" = "One\n(Generalists)", "two" = "Two\n(Pollen collection)")) +
  labs(color="Social configuration", y = "Number of individuals per nest", x = "Cluster") +
  guides(fill=FALSE, color = guide_legend(override.aes = list(size = 5)))
G4
...
## analyze counts of specialists
 behavior alone significantly predicts number of specialists per nest
```{r}
paste(nrow(count_data[count_data$specBehav == "nectarforaging",]), "shannon nectar collectors from", length(unique(count_data$nestID[count_data$specBehav == "nectarforaging"])), "nests")

paste(nrow(count_data[count_data$specBehav == "pollenforaging",]), "shannon pollen collectors from", length(unique(count_data$nestID[count_data$specBehav == "pollenforaging"])), "nests")

paste(nrow(count_data[count_data$specBehav == "feedsbrood",]), "shannon brood feeders from", length(unique(count_data$nestID[count_data$specBehav == "feedsbrood"])), "nests")

paste(nrow(count_data[count_data$specBehav == "egglaying",]), "shannon egg layers from", length(unique(count_data$nestID[count_data$specBehav == "egglaying"])), "nests")

paste(nrow(count_data[count_data$specBehav == "generalist",]), "shannon generalists from", length(unique(count_data$nestID[count_data$specBehav == "generalist"])), "nests")

paste(nrow(count_data[count_data$specBehav == "lazy",]), "shannon other from", length(unique(count_data$nestID[count_data$specBehav == "lazy"])), "nests")
...
shanBehav$nestTYPE <- as.character(shanBehav$nestTYPE)
...
shanBehav_nestSum$nestTYPE <- ordered(shanBehav_nestSum$nestTYPE, levels = c("NT", "QT", "NF", "QF"))
shanBehav_nestSum$behav <- ordered(shanBehav_nestSum$behav, levels = c("el", "bf", "nf", "pf", "gen", "oth"))
...
G2 <- ggplot(shanBehav_nestSum, aes(behav, number, color = nestTYPE)) + 
  ...
  stat_summary(fun = mean, geom = "point", size = 4, shape = 21, aes(fill = nestTYPE), color = "black") +
  scale_fill_manual(values = c("#fdb863", "#e66101", "#b2abd2", "#5e3c99")) +
  ggtitle("Shannon-threshold categorization") +
  scale_x_discrete(labels=c("el" = "Egg\nlaying", "bf" = "Brood\nfeeding", "nf" = "Nectar\ncollection", "pf" = "Pollen\ncollection", "gen" = "Generalist", "oth" = "Other")) +
  labs(color="Social configuration", y = "Number of individuals per nest", x = "Task") +
  guides(fill=FALSE, color = guide_legend(override.aes = list(size = 5)))
G2

shanBehav_nestSum$behav <- as.character(shanBehav_nestSum$behav)
...
posthoc2 <- glht(mymodel, linfct = mcp(nestTYPE = "Tukey"))
summary(posthoc2)
...
## replacement nests different?
is there a difference between bees in nests with or without replacements?
```{r} 
# define nests with replacement bees
shanBehav$replaceNest <- 0

for(i in 1:nrow(shanBehav)) {
  for(j in 1:nrow(shanBehav)) {
    if(shanBehav$nestID[i] == shanBehav$nestID[j]) {
      if(!is.na(shanBehav$addDate[j])) {
        shanBehav$replaceNest[i] <- 1
      }
    }
  }
}

# does shannon threshold specialization category differ in original vs. replacement bees?
chisq.test(shanBehav$specBehav, shanBehav$beeType)
 
# remove replacement bees from next analysis
shanBehav_orig <- shanBehav[is.na(shanBehav$addDate),]

# Does shannon threshold specialization category of original bees 
# differ based on whether there was a replacement of a dead bee in the nest?
chisq.test(shanBehav_orig$specBehav, shanBehav_orig$replaceNest)
```

# PLOT # of bees in each category
```{r}
...
# includes only known bees (rewrite count_3 to include all categorical frameworks for plots)
count_3 <- count_data[count_data$lazy == "notLazy",]
...
p3 <- ggplot() + 
  geom_point(data=nmds_data, aes(x=NMDS1, y=NMDS2, color = ovSavg), size = 2.5, position = position_jitter(width=0.05, height=0.05), alpha = 0.7) + 
...
p4 <- ggplot() + 
  geom_point(data=nmds_data, aes(x=NMDS1, y=NMDS2, color = size), size = 2.5, position = position_jitter(width=0.05, height=0.05), alpha = 0.7) + 
...
p2 <- ggplot() + 
  ...
  scale_shape_manual(values = c(21, 25), 
                     labels = c("1 - Generalist", "2 - Pollen collection")) +
  scale_fill_manual(values = alpha(c("#F8766D", "#7CAE00", "#00BFC4", "#FFFFFF"), 0.5), 
  ...
  geom_circle(data = as.data.frame(matrix(data = c(0.53, 0.8), nrow = 1)), # brood feeding 
              aes(x0 = V1, y0 = V2, r = 0.13), color = "gray") +
  geom_circle(data = as.data.frame(matrix(data = c(-1.2, 0.25), nrow = 1)), # nectar collection
              aes(x0 = V1, y0 = V2, r = 0.13), color = "gray") +
  geom_circle(data = as.data.frame(matrix(data = c(1.4, -0.95), nrow = 1)), # egg laying
              aes(x0 = V1, y0 = V2, r = 0.13), color = "gray") +
  geom_text(data=behav_scores, aes(x=NMDS1, y=NMDS2, label=behav), size = 2.5) +
  coord_cartesian(xlim=c(-1.5, 1.5), ylim=c(-1.3, 1.3)) +
  ...
  guides(color = guide_legend(override.aes = list(shape = 22, size = 5)), 
         fill = guide_legend(override.aes = list(shape = 22, size = 5, 
                                                 fill = alpha(c("#F8766D", "#7CAE00", "#00BFC4", "#FFFFFF"), 0.4))))
...
4. CORRELATIONS BETWEEN BEHAVIOR & BODY SIZE / OVARY DEV -- included above in categorical task frameworks
    "body size & ovary development models" chunks
...
# 5.1 RPTR
use count_3Time (exclude workers with <3 obs)
...
## prep dataframe
```{r}
rptdata <- count_3Time %>% dplyr::select(nestID, nestTYPE, beeID, timepoint, nectarforaging, feedsbrood, pollenforaging, nBehavs, lazy)
...
# rptr analysis is very slow - uncomment to run
...
```
 ...
 change in nest from early to late
```{r}
# summarize by nest
...
# multiply proportion by 100 to turn it into a percent
...
# 5.2 & 5.3 SHANNON VALUES
use count_time (include workers with <3 obs) <- to assign NA shannon to these workers

### calculate shannon at early and late
...
count_3Time <- count_time[!is.na(count_time$shannon),] 
print(paste(nrow(count_3Time[count_3Time$timepoint == "early",]), "early bees that meet the shannon threshold", sep = " "))
print(paste(nrow(count_3Time[count_3Time$timepoint == "late",]), "late bees that meet the shannon threshold", sep = " "))
...
# model 0 vs. >0 shannon values
ps1 <- glmer(shan0 ~ timepoint + (1|nestTYPE:nestID:beeID) + (1|natal) + (1|nBehavs),
...
ps1 <- glmer(shan1 ~ timepoint + (1|nestTYPE:nestID:beeID) + (1|natal) + (1|nBehavs),
              data = count_3Time)
...
nmds_time<- count_3Time %>% 
  dplyr::select(nestID, nestTYPE, beeID, caste,  
         timepoint, nectarforaging, feedsbrood, pollenforaging, lazy)

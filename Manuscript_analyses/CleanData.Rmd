...
## erica's relative path
filenames <- list.files("/Users/ericasarro/Google\ Drive\ File\ Stream/My\ Drive/WoodardLab.shared/Projects/2018.Awesome.Summer.Experiments/Raw\ Data/BORISfiles",
                        pattern = "*.csv",
                        full.names = TRUE)

## kaleigh's relative path
filenames <- list.files("/Users/kaleighfisher/Desktop/Boris_files", 
                        pattern="*.csv",
                        full.names = TRUE)
...
all_data <- do.call(rbind(borisfiles))

all_data

## do this to all the things 
#all_data <- dataframe()
#for (i in 1:length(borisfiles)) {
 # all_data <- rbind(all_data, borisfiles[i])
#}
#do.call(rbind(borisfiles))
```
...
## set wd to folder containing BORISfiles folder so we can access from different computers
## for some reason this is not working, so i put the whole path in the load
setwd("/Users/ericasarro/Google\ Drive\ File\ Stream/My\ Drive/WoodardLab.shared/Projects/2018.Awesome.Summer.Experiments/Raw\ Data")

getwd()

## load a subset of data to test
all_data <- read.csv("/Users/ericasarro/Google\ Drive\ File\ Stream/My\ Drive/WoodardLab.shared/Projects/2018.Awesome.Summer.Experiments/Raw\ Data/BORISfiles/NF02.csv", 
                     header = TRUE, 
                     col.names = c("obsID", 
                                   "obsDate", 
                                   "filename", 
...
#WRITE CODE HERE TO CHECK STATUS OF OBSERVATIONS
```

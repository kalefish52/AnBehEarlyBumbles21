...
recordedBees <- dplyr::select(obs_data, nestID, beeID)
## remove duplicates
...
## select only relevant information
masterBees <- dplyr::select(masterBees, nestID, IDnum, caste)
...
## select only relevant information
allBees <- dplyr::select(obs_data, nestID, beeID, caste)
## remove duplicates
...
count_data <- count_data %>%
  dplyr::select(nestID, beeID, behav, totCount, caste)
...
data_sum <- count_data %>% dplyr::select(nestID, nestTYPE, nectarforaging, pollenforaging, feedsbrood, fillshoneypot, laysegg) %>% arrange(nestID)
...
countsDATA <- counts_final %>%
  dplyr::select(nestID, nestTYPE, beeID, caste, nBees, nWork, timepoint, feedsbrood, nectarforaging, pollenforaging, egglaying, nBehavs)

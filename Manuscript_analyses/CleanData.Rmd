...
## pull out caste from beeID column
...
## pull out caste from masterBees
...
# #So we can add caste information to the counts data 
# CasteInfo <- select(masterBees, nestID, IDnum, IDtype)
# CasteInfo

## select only relevant information
masterBees <- select(masterBees, nestID, IDnum, caste)
...
## select only relevant information
allBees <- select(obs_data, nestID, beeID, caste)
## remove duplicates
...
count_data <- count_data %>%
  pivot_wider(id_cols = c("nestID", "beeID", "caste", "behav"), 
              names_from = rand, 
              values_from = count) 

## add column to fill with sum of counts for each behav
count_data$totCount <- rowSums(count_data[5:ncol(count_data)], na.rm = TRUE)
count_data
## subset dataframe so that all is left is nestID, beeID, behav, totCount, and caste
count_data <- count_data %>%
  select(nestID, beeID, behav, totCount, caste)
...
count_data$nBehavs <- rowSums(count_data[, 4:ncol(count_data)], na.rm = TRUE)
# 
...
count_data <- count_data[!count_data$caste == "U", ]
count_data <- count_data[!count_data$caste == "N", ]
...
# }
for (i in 1:nrow(count_data)) {
  if (count_data$caste[i] == 1 || 
      count_data$caste[i] == 2 || 
      count_data$caste[i] == 3 ||
      count_data$caste[i] == 4 ||
      count_data$caste[i] == 5 ||
      count_data$caste[i] == 6 ||
      count_data$caste[i] == 7 ||
      count_data$caste[i] == 8 ||
      count_data$caste[i] == 9) {
    count_data$caste[i] <- 0
  }
}
...
eggsmergebind<-eggsmerge %>% rename(beeID=WorkerID) %>% rename(caste=Caste) %>% select(nestID, beeID, caste, timepoint, behav, dateID)
head(eggsmergebind)
head(nestDEV)
nestDEVbind<-nestDEV %>% select(nestID, dateID, timepoint, behav, beeID, caste) 
nestDEVbind
...
nestDEVbind<-nestDEVbind %>% select(nestID, dateID, timepoint, behav, beeID, caste)
...
counts_final <- counts_final %>%
  select(nestID, beeID, caste, timepoint, behav, totCount)
...
queenlessChar <- sapply(strsplit(obs_data$nestID, ""), 
                          function(x){
                            x[1]
                          })
numberChar <- sapply(strsplit(obs_data$nestID, ""),
                        function(x){
                          x[2]
                        })    
counts_final$nestTYPE <- paste0(queenlesChar, numberChar)

## subset to remove the behaviors we're not interested in (behavs only used in erica's time budgeting)
countsDATA <- counts_final %>%
  select(nestID, nestTYPE, beeID, caste, timepoint, feedsbrood, nectarforaging, pollenforaging, egglaying, nBehavs)

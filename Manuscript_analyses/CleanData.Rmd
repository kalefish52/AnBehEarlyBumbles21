...
for(i in 1:length(unique(egglaying$nestID))) {
  ...
  for(j in 1:nrow(nestidEggs)){
    midpoint<-((max(nestidEggs$dateID[j])-min(nestidEggs$dateID[j]))/2)+min(nestidEggs$dateID[j])
    for(p in 1:nrow(nestidEggs)){
      if (nestidEggs$dateID[j]<= midpoint[j]){
        eggsearly<-rbind(eggsearly, nestidEggs[j,])
      }
      else if (nestidEggs$dateID[j]>midpoint[j]){
        eggslate<-rbind(eggslate, nestidEggs[j,])
      }
      else {
        eggserror<-rbind(eggserror, nestidEggs[j,])
      }
    }
  }}
...
(max(lala$dateID)-min(lala$dateID)/2)+min(lala$dateID)

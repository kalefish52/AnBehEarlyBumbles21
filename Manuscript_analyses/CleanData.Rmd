...
```{r}
# Add early/late column to obs_data using join 
head(obs_data)

#Make obs_data into new dataframe so we don't interfere in upstream code
nestDEV<-obs_data

# make timeID column in obs_data
nestDEV$camID <- sapply(strsplit(nestDEV$obsID, "\t"), 
                          function(x){
                            x[1]
                            } )
## element 3 is dateID
nestDEV$dateID <- sapply(strsplit(nestDEV$obsID, "\t"), 
                          function(x){ 
                            x[3]
                            } )
## element 4 is hourID
nestDEV$hourID <- sapply(strsplit(nestDEV$obsID, "\t"), 
                          function(x){ 
                            x[4]
                            } )

## combine date and hour into a single column called timeID
nestDEV$timeID <- paste(nestDEV$dateID, nestDEV$hourID)

## add early/late time information 

...
R1<-ggplot(rpttime, aes(Dist, Behavior)) +
  geom_point(position = position_jitter(width=2, height=0.3), alpha = 0.6, aes(color = nestTYPE, size = abs(propDist), shape = inf)) +
  ...
  labs(x = "Change in number of behaviors performed by each individual", y = "Task", 
       color = "Social\nconfiguration", size = "Percent change", shape = "Zero observations\nin one timeframe") +
...
R2<-ggplot(rptnest, aes(nestDist, Behavior)) +
  ...
  scale_shape_manual(values = c(19, 21)) +
  #stat_summary(fun = mean, geom = "point", size = 3) + 
  labs(x="Change in number of behaviors performed in each nest", y = "Task", 
       size = "Percent change", color = "Social\nconfiguration", shape = "Zero observations\nin one timeframe") +
  scale_y_discrete(labels=c("pollenforaging" = "Pollen\ncollection", "nectarforaging" = "Nectar\ncollection", "feedsbrood" = "Brood\nfeeding"))
R2
ggsave("../figures/rptNEST.jpg")

  ...
  # look at the the late bees only
    ...
    # then iterate back through the dataset
      ...
      # and find the early entry with the same behav
        ...
        # calculate the percent change (early - late) / early
      }
    }
  }
}
...
rpttime
rptworkers
# add column for proportion change from early to late
...
R1<-ggplot(rpttime, aes(propDist, Behavior)) +
  ...
  labs(x = "Percent change in number of behaviors performed by each individual", y = "Task", color = "Social\nConfiguration") +
...
R2<-ggplot(rptnest, aes(nestDist, Behavior)) +
  geom_point(position = position_jitter(width=2, height=0.3), alpha = 0.6,
             aes(color = nestTYPE)) +
  scale_color_brewer(palette = "PuOr", labels = c("W3", "W5", "QW3", "QW5")) +
  stat_summary(fun = mean, geom = "point", size = 3) + 
  labs(x="Change in Number of Behaviors Performed in Each Nest", y = "Task", color = "Social\nConfiguration") +
  scale_y_discrete(labels=c("pollenforaging" = "Pollen\nCollection", "nectarforaging" = "Nectar\nCollection", "feedsbrood" = "Brood\nFeeding"))
R2
ggsave("../figures/rptNEST.jpg")

...
```{r}
count_data <- read.csv(paste(git_filepath, "/Manuscript_analyses/count_data.csv", sep = ""))
dur_data <- read.csv(paste(git_filepath, "/Manuscript_analyses/dur_data.csv", sep = ""))
camDur <- read.csv(paste(git_filepath, "/Manuscript_analyses/camDur.csv", sep = ""))
```
...
gath_count
```
pie charts with propotional counts of behaviors
```{r}
...
figure
```
bar graphs with counts of each behavior. bar height = mean, error = sem, grouped by behavior, colored by social group
```{r}
...
### Analyze Data
```{r}
## add natal colonies and wing size to dataframe

## import master data sheets that contain natal colonies
...
filenames <- list_filenames("/Users/ericasarro/Google\ Drive\ File\ Stream/My\ Drive/WoodardLab.shared/Projects/2018.Awesome.Summer.Experiments/Raw\ Data/MasterDatasheets")

filenames

## loop over the files and import all of them. rename column names to remove "." and make shorter
master_data <- do.call(rbind, lapply(filenames, 
                                    function(i) {
                                      read.csv(i, 
                                               header = FALSE, 
                                               skip = 1,
...
## add natal colonies to queen_dur file
master_data <- subset(master_data, nestID %in% queen_dur$nestID)

## sort master_data and queen_dur so I can combine
master_data$nestID <- as.character(master_data$nestID)
master_data <- master_data[order(master_data$nestID),]
queen_dur$nestID <- as.character(queen_dur$nestID)
queen_dur <- queen_dur[order(queen_dur$nestID),]
## combine
queen_dur <- cbind(master_data$Qnatal, master_data$Wnatal, queen_dur)
## rename added columns
names(queen_dur)[1] <- "Qnatal"
names(queen_dur)[2] <- "Wnatal"

## differences between duration of each behavior across social groups + time
queen_dur

## linear mixed model
# *******need to add in WingAvg**********
# *******how do i run the model to include values that have NA for a random effect?********
noTime_incubateWnat <- lme4::lmer(incubate ~ treat + (1|Qnatal) + (1|Wnatal), data = queen_dur)

noTime_incubate <- lme4::lmer(incubate ~ treat + (1|Qnatal), data = queen_dur)
noTime_incubate
noTime_feedsbrood <- lme4::lmer(feedsbrood ~ treat + (1|Qnatal), data = queen_dur)
noTime_feedsbrood
noTime_nectarforaging <- lme4::lmer(nectarforaging ~ treat + (1|Qnatal), data = queen_dur)
noTime_nectarforaging
noTime_pollenforaging <- lme4::lmer(pollenforaging ~ treat + (1|Qnatal), data = queen_dur)
noTime_pollenforaging

#acrossTime <- lme4::lmer(incubate ~ treat + WingAvg + (nestID|NatCol) + (nestID|WNatCol), data = queen_dur)

```


=======
>>>>>>> c3fcf4ed9d4ae9264799c2f85c757203b0f4b011

...
queen_data$treat <- factor(queen_data$treat, levels = c("QA", "QT", "QF"))

## make new column for total time spent doing nothing
## ********make more inclusive of all behaviors************
queen_data$totalLen <- as.numeric(queen_data$totalLen)
queen_data$non <- NA
for (i in 1:nrow(queen_data)) {
  queen_data$non[i] <- queen_data$totalLen[i] - 
                          queen_data$feedsbrood[i] - 
                          queen_data$pollenforaging[i] - 
                          queen_data$nectarforaging[i] 
}
...
bp <- ggplot(dfSumm, aes(x="", y=mean, fill=behav)) +
...
```{r}
## call it abs_data to make distinct
abs_data <- gath_data
